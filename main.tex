...
\section{Introduction}
    ...
    \end{equation*}
    The name stems from the smoothing effect it has on the solution {\color{red}. Pioneering connections between quadratic regularization and spline smoothing were presented in \cite{Kimeldorf1970}, involving Bayesian estimation of Gaussian stochastic processes.} Early {\color{red} applied} versions {\color{red} of a quadratic regularization} include Tikhonov regularization~\cite{tikhonov1963solution}, known as ridge-regression in statistics~\cite{Hoerl1962ridge}. Smooth regularization has been widely used over more than 70 years, with countless applications. %with applications on discrete domain problems as well as recovery in the continuum \ad{ref needed here?}.
    ...
    Solving such a sparsity-promoting penlized problem is generally more difficult than the case of quadratic regularization on a Hilbert space: the solution set may be infinite-dimensional and the non-reflexivity of the Banach space prevents us from using gradient steps.
    ...
    {\color{red} Find the critera for $\mathcal{R}_1$.}
...
\section{Sparse-plus-Smooth Composite Representer Theorem}
   ...
   %  \end{equation*}
    ...
    \begin{proof}
        ...
        Conversely, let us assume $0 < \lambda < \lm$ and let us prove the second statement. We want to show that $0$ is not solution, or equivalently that $\frac{1}{2} \norm{\phib(s)}_2^2 - \langle \phib^*\bm{y}, s \rangle_{\cA \times\cB} + \lambda \norm{s}_{\cB}$ can be negative. Let us write $v = \phib^*\bm{y} \in \cA$. Using the Hanh-Banach theorem \cite[Chapter~1]{Brezis_2011}, there exists an element $s_0\in\cB$ such that $\norm{s_0}_\cB = \norm{v}_\cA$ and
        \begin{equation*}
            \langle v, s_0\rangle = \norm{v}_\cA^2 = \norm{s_0}_\cB \norm{v}_\cA.
        \end{equation*}
        We can then write
