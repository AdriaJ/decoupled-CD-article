...
\begin{abstract}
    We consider composite linear inverse problems where the signal to recover is modeled as a \ad{sum} of two functions. 
...

{
    ...
    Remaining todo's:
    \begin{itemize}
        \item Check consistency for solution of optimization problems: star, hat or something else? (like five-arms star).
        \item Proof of Theorem 2 to be deferred in the appendix.
        \item Acknowledge the reviewers.
        \item Mention the assumption about boundedness of $\phib$.
        \item Homogenize the use of \/emph (italic)
        \item Homogenize the use of $s_1^*$ or $\hfb$ (stick to the second version probably)
        \item Display of the figures in the appendix (cross tables in particular)
        \item Capitalization of Figures, Section, ... ?
        \item homogeneity of `single-component'
    \end{itemize}
}
...
\section{Introduction}
    ...
    In a theoretical model, the addition between $s_1^\dagger$ and $s_2^\dagger$ may not be properly defined because the signals may belong to different spaces. For this reason, in what follows we consider the more general inverse problem over the pair $(s_1^\dagger, s_2^\dagger)$ with two possibly different operators
    \begin{equation}
    ...
    \begin{equation} \label{eq:compositepb}
        (s_1^*,s_2^*) \in \underset{(s_1,s_2)}{\arg\min} \ \mathcal{D}(\bm{y}, \bm{\Phi}_1(s_1) + \bm{\Phi}_2(s_2)) + \mathcal{R}_1 (s_1) + \mathcal{R}_2(s_2),
    \end{equation}
    ...
    \end{equation}
    which solves a non-composite inverse problem.
    ...
    \end{equation*}
    Smooth regularization has been widely used over more than 70 years, with countless applications. The name stems from the smoothing effect it has on the solution. Pioneering connections between quadratic regularization and spline smoothing were presented in \cite{Kimeldorf1970}, involving Bayesian estimation of Gaussian stochastic processes. Early applied versions of a quadratic regularization include Tikhonov regularization~\cite{tikhonov1963solution}, known as ridge-regression in statistics~\cite{Hoerl1962ridge}.  %with applications on discrete domain problems as well as recovery in the continuum \ad{ref needed here?}.
    Hilbertian penalization is strongly connected to the RKHS theory and thus there exist representer theorems that precisely specify the form of the solution~\cite{Scholkopf2001generalized}. Interestingly, these problems are fairly simple to solve as they can be recast into a finite-dimensional formulation, even when the signal $s$ to recover is continuously defined, as for instance with $\mathcal{H} = \ldx$ the space of square integrable functions over a domain $\cX$. 
    ...
    The second approach considered in this work is the one of sparsity-promoting regularizations
    \begin{equation*}
        \mathcal{R}(s) = \lambda \mathcal{R}^\mathrm{S}(s).
    \end{equation*}
    In this scenario, we consider the theoretical model of signals $s$ living in a Banach search space $\cB$, and a convex function $\cRS: \cB \to \R_+$. This framework is more general than the former case of Hilbert spaces and thus enables us to cover most of the common sparsity-promoting penalties.
    The classical finite-dimensional example for $\mathcal{R}^\mathrm{S}(\cdot)$ is the $\ell_1$-norm over Euclidian $n$-spaces \cite{tibshirani1996regression,chen2001atomic}, optionally including an analysis prior model with a nested linear operator \cite{elad2006analysis}. The continuous-domain counterparts consist in the problems of sparse spikes recovery \cite{laville2021sparse} and spline reconstruction \cite{unser2017splines}, both being infinite-dimensional and based on a total-variation penalty. When a Banach norm $\mathcal{R}^\mathrm{S} (s) = \| s \|_{\mathcal{B}}$ is considered, the solution set of the single-component problem \eqref{eq:singletpb} is described thanks to representer theorems: the solutions are expressed as sums of extreme points of the Banach norm unit ball \cite{boyer2019representer,Unser2020,bredies2020sparsity}. Similarly, $\mathcal{R}^\mathrm{S}(\cdot)$ can be defined as an atomic norm to promote sparse solutions, as the extreme points are generally sparse vectors \cite{chandrasekaran2012convex}. This framework of Banach search space also covers significantly more complex functional setups of sparse recovery, as for instance in \cite{laville2023a,ambrosio2023,decastro2024}.
    Solving such a sparsity-promoting penalized problem is generally more difficult than the case of quadratic regularization on a Hilbert space: the solution set may be infinite-dimensional and the non-reflexivity of the Banach space prevents us from using gradient steps.
...
\section{Representer Theorems for Single-Component Problems}
   ...
   Proposition \ref{prop:hilbertRT} hereafter is a classical result for quadratic optimization over Hilbert spaces. Significantly more general formulations can be found in~\cite{Scholkopf2001generalized} but we restrict here to the case which is relevant for our purpose. We follow the exposition of~\cite[Theorem~7]{caponera2021nonparametric}, which is based on \cite[Section~3.2]{Unser2020}. 
    ...
    \ad{Should I use a proper star instead of the product star as exponent ?}
...
\section{Sparse-plus-Smooth Composite Representer Theorem}
   ...
   %  \end{theorem}
    ...
    \begin{proof}
        ...
        \begin{equation*}
            \forall s \in \cB, \qquad \mathcal{J}_\mathcal{B}(s) \geq \mathcal{J}_\mathcal{B}(0),
        \end{equation*}
        hence the null element is solution, $0 \in \mathcal{V}(\lambda)$. Moreover, the strict convexity of the square $\ell_2$-norm in the data-fidelity term induces uniqueness of the fit \cite{tibshirani2013lasso}, that is any solutoin belongs to the nullspace of the measurement operator $\phib$, as follows
        \begin{equation*}
            \forall s \in \mathcal{V}(\lambda), \qquad \phib(s) = \phib(0) = 0.
        \end{equation*}
        Necessarily, $0$ is the unique solution in the case $\lambda \geq \lm$, which proves the first statement.
        ...
        \end{equation}
        Using asymptotic growth comparison toward $0$, there exists $\xi > 0$ such that
        \begin{equation*}
            \frac{1}{2} \norm{\phib(\xi s_0)}_2^2 - \langle \phib^*\bm{y}, \xi s_0 \rangle_{\cA \times\cB} + \lambda \norm{\xi s_0}_{\cB} < 0,
        \end{equation*}
        which proves that $0$ is not a minimum.
    \end{proof}
...
\label{sec:application}
    ...
    \subsection{Composite representer theorem}
        ...
        \begin{proof}
            By construction, Corollary~\ref{cor:main} holds for problem \eqref{eq:argmin-sps}. It directly leads to the expression of $s_2^*$ in $\eqref{eq:deconv-hilbertpart}$. Regarding the sparse component, we have that
            \begin{equation*}
...
\section{Benefits of our Decoupled Composite Model}
    ...
    \label{sec:bene:deco}
        ...
        For a fair treatment between the solver, the stopping criterion of all the algorithms is set as a threshold on the relative improvement of consecutive iterates of the foreground component variable.
        \ad{Moreover, comment on the technical implementaiton choices to remain fair.}


        We present two experiment: the first one reports the reconstruction time when the contrast $r_{1/2}$ evolves, in the same setup as in section~\ref{sec:bene:comp}; the second experiment presents the reconstruction time when the super-resolution factor $n_\mathrm{srf}$ varies. The results, respectively reported in figure~\ref{fig:time-vs-r} and figure~\ref{fig:time-vs-srf}, correspond to the duration of the best reconstruction obtained through a tested set of regularization parameters. Each experiment is reproduced 24 times, the median value is reported with the solid line and the shaded area represents the interquartile spread.
