...
\section{Sparse-plus-Smooth Composite Representer Theorem}
   ...
   %  \end{equation*}
    ...
    For $\lambda >0$, we define the objective functional $\mathcal{J}_\mathcal{B}(s) := \frac{1}{2} \norm{\bm{y} - \bm{\Phi}(s)}_2^2 + \lambda \norm{s}_{\cB}$ and we consider the solution set of the following optimization problem
    \begin{equation}
        \label{eq:single-banach}
        \mathcal{V}(\lambda) := \underset{s \in \cB}{\arg \min} \mathcal{J}_{\mathcal{B}}(s).
    \end{equation}
    Let us define
    \begin{equation}
    ...
    \begin{itemize}
        \item[1.] For any $\lambda \geq \lm$, the solution to \eqref{eq:single-banach} is unique and the solution set is reduced to $\mathcal{V}(\lambda) = \left\{0\right\} \subset \cB$.
        \item[2.] For any $\lambda < \lm$, the null element $0$ is not solution, i.e., $0 \notin \mathcal{V}(\lambda)$.
    \end{itemize}
    \end{proposition}
    ...
    \ad{rewrite this as a corollary}
