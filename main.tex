...
\section{Introduction}
    ...
    As $s_1^\dagger$ and $s_2^\dagger$ may belong to different spaces for which the addition would be impossible/incompatible/undefined, from now on we consider the more generic inverse problem over the pair $(s_1^\dagger, s_2^\dagger)$ with two possibly different operators
    \begin{equation}
    ...
    % \ad{Maybe take a bit of distance with Banach norms, and promote instead atomic norms. Classical example is Banach norm over Banach space, but other penalties may be used for sparse reconstruction. NEED a notation! $\mathcal{R}_1$ or $\mathcal{R}_\mathrm{SPARSE}$ or $\mathcal{R}^\mathrm{S}$ ?}
    % {\color{red} Use a notation for sparsity promoting norms ($\mathcal{R}^\mathrm{S}$ for instance), keep in mind the classical case of Banach norms $\norm{\cdot}_\mathcal{B}$ and mention that there exist other forms. Cf Boyer ?}
    ...
    {\color{red} Find the criteria for $\mathcal{R}_1$.}
...
\section{Representer Theorems for Single-Component Problems}
   ...
   Proposition \ref{prop:hilbertRT} hereafter is a classical result for quadratic optimization over Hilbert spaces. Significantly more general formulations can be found in~\cite{Scholkopf2001generalized} but we restrict here to the case which is relevant for our purpose. We follow the exposition of~\cite[Theorem~7]{caponera2021nonparametric}, which is based on \cite[Section~3.2]{Unser2020}. 
    ...
    The classical framework for sparse reconstruction instead considers a search space that is a generic non-reflexive Banach space $\mathcal{B}$ and assumes the existence of a predual space \cite{unser2021unifying}.
...
\section{Sparse-plus-Smooth Composite Representer Theorem}
    ...
    {\color{red} Unify notation of \eqref{eq:compositepb} and here, remove the $+$ sign between the two components.}
   ...
   %  \end{equation*}
    ...
    {\color{red} We first extend this result to the generic case of single-components problems penalized with a Banach norm in the following proposition.
    \begin{proposition}[Bound on single-component Banach-penalized problems]
    ...
    \ad{Rewrite this as a corollary applied to the case $\norm{\cdot}_\cB$ penalization.}
...
\label{sec:application}
    ...
    \label{sec:comp:model}
        ...
        \ad{Temper the claim: "we use a simplistic scenario involving linear operator and continuous-domain reocnstruction. This kind of scenario would be relevant for instance in SMLM imaging."}
